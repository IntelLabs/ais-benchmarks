...
class CJSDivergence(CDivergence):
    ...
    def compute_from_samples(self, p, q, samples):
        ...
        # Filter zero prob values and nans on any of the sides
        inliers = np.logical_and(p_samples_prob > 0, q_samples_prob > 0)
        inliers = np.logical_and(inliers, np.logical_not(np.isnan(q_samples_prob)))
        ...
        # Remove nans that might have been generated by underflows
