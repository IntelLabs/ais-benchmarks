...
class CTreePyramidNode:
    def __init__(self, center, radius, node_idx, leaf_idx, level, kernel="haar", bw_div=4, weight=t_tensor(0)):
        ...
        if kernel == "haar":
            ...
            self.sampler = CMultivariateNormal({"mean": self.center,
                                                "sigma": np.diag(
                                                    np.ones_like(self.center) * (self.radius / self.bw_div))
                                                })
        else:
    ...
    def sample(self, sampler):
        ...
        self.coords_hist = np.concatenate(
            (self.coords_hist, self.coords)) if self.coords_hist is not None else self.coords
    ...
    def weigh(self, target_d, importance_d, target_f=lambda x: 1):
        ...
        self.weight_hist = np.concatenate(
            (self.weight_hist, self.weight)) if self.weight_hist is not None else self.weight
    ...
    def __repr__(self):
        return "[ l_idx:" + str(self.leaf_idx) + " , nidx:" + str(self.node_idx) + " , c:" + str(
            self.center) + " , r:" + str(self.radius) + " , w:" + str(self.weight) + " ]"
...
class CTreePyramidSampling(CMixtureISSamplingMethod):
    ...
    def draw(self, ax):
        ...
        if self.ndims == 1:
            ...
            res = self.draw_3d_tree(self.T, ax)
        return res
    ...
    @staticmethod
    def draw_3d_tree(T, ax, facecolor=(1, 1, 1), edgecolor=(0, 0, 0), alpha=0.4):
        res = []
        for l, w in zip(T.leaves, T.weights):
            c = l.center
            r = l.radius
            # w = l.value
            lines = np.array([[c[0] - r, c[1] - r, c[2] - r],
                              [c[0] + r, c[1] - r, c[2] - r],
                              [c[0] + r, c[1] + r, c[2] - r],
                              [c[0] + r, c[1] + r, c[2] + r],
                              [c[0] - r, c[1] + r, c[2] + r],
                              [c[0] - r, c[1] - r, c[2] + r],
                              [c[0] - r, c[1] - r, c[2] - r]]).reshape(7, 3)

            res.extend(ax.plot3D(xs=lines[:, 0], ys=lines[:, 1], zs=lines[:, 2], alpha=alpha,
                                 color=cm.hot(w), linewidth=1, ls="--"))
    ...
    def _self_normalize(self):
        if np.sum(self.T.weights[self.T.leaves_idx]) > 0:
            self.T.weights[self.T.leaves_idx] = self.T.weights[self.T.leaves_idx] / np.sum(
                self.T.weights[self.T.leaves_idx])
