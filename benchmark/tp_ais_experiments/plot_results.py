...
def make_plots(results_file, methods, labels, output_path, dimensions=[1, 2, 3], dists=["gmm"]):
    # dims samples kl_kde bhat_kde kl_nn bhat_nn time method final_samples
    data = pd.read_table(results_file, sep=" ", index_col=False, skipinitialspace=True)
    path = output_path
    ...
    # methods = ["TP_AISr_ESS95", "TP_AISr", "TP_AIS"]

    # TP-AIS vs. HiDaisee comparison
    # methods = ["hi_daisee", "tp_simple_leaf_haar"]
    # labels = ["HiDaisee ESS60", "TP_AIS (ours)"]

    # dims output_samples JSD BD ev_mse NESS time method target_d accept_rate proposal_samples proposal_evals target_evals
